...
def analyze_speedup():
    ...
    files_filtered = [
        "main_res_filtered.csv",
        "cusparse_performance_results_filtered.csv", 
        "sputnik_performance_results_filtered.csv",
        "SparTA_v100_performance_results_filtered.csv"
    ]
    ...
    for N in Ns:
        ...
        print(f"有效配置数: {configs_with_data}")
    ...
    # 显示一些具体的例子（现在应该更合理）
    print("\n具体示例 (M=4096, K=4096, 过滤后):")
    print("=" * 50)
    
    for N in [8, 16]:  # 只显示N=8和N=16，因为N=32可能被过滤了
        for sparsity in [40, 70]:

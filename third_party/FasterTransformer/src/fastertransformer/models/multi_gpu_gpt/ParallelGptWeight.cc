    ...
    gpt_variant_params_(gpt_variant_params)
{
    ...
    //                                     -> ffn_weights.output_weight
    // 每个矩阵的input数组的长度
    std::vector<int> val_count_List_AllLayers = std::vector<int>(num_layer_ * 4);
    std::vector<int> num_gtiles_List_AllLayers = std::vector<int>(num_layer_ * 4);
    std::vector<int> num_mtiles_List_AllLayers = std::vector<int>(num_layer_ * 4);
    std::vector<int> num_ltiles_List_AllLayers = std::vector<int>(num_layer_ * 4);
    std::vector<int> SplitK_List_AllLayers = std::vector<int>(num_layer_ * 4);
    loadDataArrayFromBin_To_CPU<int>(val_count_List_AllLayers.data(),
                                     num_layer_ * 4,
                                     dir_path + "/val_count_List." + std::to_string(tensor_para_rank_) + ".bin");
    loadDataArrayFromBin_To_CPU<int>(num_gtiles_List_AllLayers.data(),
                                     num_layer_ * 4,
                                     dir_path + "/num_gtiles_List." + std::to_string(tensor_para_rank_) + ".bin");
    loadDataArrayFromBin_To_CPU<int>(num_mtiles_List_AllLayers.data(),
                                     num_layer_ * 4,
                                     dir_path + "/num_mtiles_List." + std::to_string(tensor_para_rank_) + ".bin");
    loadDataArrayFromBin_To_CPU<int>(num_ltiles_List_AllLayers.data(),
                                     num_layer_ * 4,
    ...
    FT_CHECK_WITH_INFO(val_count_List_AllLayers.size() == num_layer_ * 4,
                       "ERROR: val_count_List_AllLayers            should have num_layer_*4 elements!\n");
    FT_CHECK_WITH_INFO(num_gtiles_List_AllLayers.size() == num_layer_ * 4,
                       "ERROR: num_gtiles_List_AllLayers           should have num_layer_*4 elements!\n");
    FT_CHECK_WITH_INFO(num_mtiles_List_AllLayers.size() == num_layer_ * 4,
                       "ERROR: num_mtiles_List_AllLayers           should have num_layer_*4 elements!\n");
    FT_CHECK_WITH_INFO(num_ltiles_List_AllLayers.size() == num_layer_ * 4,
                       "ERROR: num_ltiles_List_AllLayers           should have num_layer_*4 elements!\n");
    FT_CHECK_WITH_INFO(SplitK_List_AllLayers.size() == num_layer_ * 4,
                       "ERROR: SplitK_List_AllLayers               should have num_layer_*4 elements!\n");
    //
    decoder_layer_weights.clear();
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            const std::vector<int> val_count_List{val_count_List_AllLayers[l * 4],
                                                  val_count_List_AllLayers[l * 4 + 1],
                                                  val_count_List_AllLayers[l * 4 + 2],
                                                  val_count_List_AllLayers[l * 4 + 3]};
            const std::vector<int> num_gtiles_List{num_gtiles_List_AllLayers[l * 4],
                                                   num_gtiles_List_AllLayers[l * 4 + 1],
                                                   num_gtiles_List_AllLayers[l * 4 + 2],
                                                   num_gtiles_List_AllLayers[l * 4 + 3]};
            const std::vector<int> num_mtiles_List{num_mtiles_List_AllLayers[l * 4],
                                                   num_mtiles_List_AllLayers[l * 4 + 1],
                                                   num_mtiles_List_AllLayers[l * 4 + 2],
                                                   num_mtiles_List_AllLayers[l * 4 + 3]};
            const std::vector<int> num_ltiles_List{num_ltiles_List_AllLayers[l * 4],
                                                   num_ltiles_List_AllLayers[l * 4 + 1],
                                                   num_ltiles_List_AllLayers[l * 4 + 2],
                                                   num_ltiles_List_AllLayers[l * 4 + 3]};
            const std::vector<int> SplitK_List{SplitK_List_AllLayers[l * 4],
                                               SplitK_List_AllLayers[l * 4 + 1],
                                               SplitK_List_AllLayers[l * 4 + 2],
                                               SplitK_List_AllLayers[l * 4 + 3]};
...
#ifdef SPARSITY_DEBUG  
    printf("Start load decoder_layer_weights!\n");
#endif
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
...
#ifdef SPARSITY_DEBUG  
    printf("Finished load decoder_layer_weights!\n");
#endif
}

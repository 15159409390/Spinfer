...
#ifdef SPARSITY_LLM
template void deviceFree(unsigned int*& ptr);
template void deviceFree(unsigned long*& ptr);
#endif
...
template int loadDataArrayFromBin<__half>(__half* TargetArrayPTR, int NumElem, std::string FileName);
template int loadDataArrayFromBin<unsigned long>(unsigned long* TargetArrayPTR, int NumElem, std::string FileName);
template int loadDataArrayFromBin<__nv_bfloat16>(__nv_bfloat16* TargetArrayPTR, int NumElem, std::string FileName);
template int loadDataArrayFromBin<int>(int* TargetArrayPTR, int NumElem, std::string FileName);
template int loadDataArrayFromBin<float>(float* TargetArrayPTR, int NumElem, std::string FileName);
template int loadDataArrayFromBin<unsigned int>(unsigned int* TargetArrayPTR, int NumElem, std::string FileName);
...
#ifdef SPARSITY_LLM
    if (ptr == nullptr) {
        printf("[SPLLM Warning] Skip loadWeightFromBinFunc: ptr is nullptr!\n");
        return 0;
    }
#endif
    if (shape.size() > 2) {
